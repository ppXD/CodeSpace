...
public sealed record JournalReviewVerdict
{
    ...
    /// <summary>The reviewer's own agent run — the frontend deep-links its terminal ("view reviewer run →").</summary>
    public required Guid ReviewerRunId { get; init; }

    /// <summary>The harness the reviewer ran on (e.g. <c>claude-code</c> when the producer ran <c>codex-cli</c>) — the independence line the card shows. Null when unknown.</summary>

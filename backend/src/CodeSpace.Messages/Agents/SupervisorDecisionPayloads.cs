...
/// <summary>
/// WHAT a server stop gate's card asked a human to adjudicate — the blocker's IDENTITY, carried as a root
...
/// blocker the latest attempt reports: same blocker ⇒ the human already ruled on it, release the stop;
/// a different one ⇒ a genuinely new question they have never seen, park on a fresh card. Shared by BOTH gates on
/// the stop — the delivery gate's publish blockers and the publish gate's own unpublished-merge blocker — because
/// a run under an immutable policy hits both, and two mirrored notions of "the same question" would dead-end it.
///

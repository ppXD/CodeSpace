...
/// gets a clean terminal instead of lingering Pending forever. Dispatched by the recurring reaper job (which always
/// runs); can also be sent ad-hoc (admin path / tests).
///
/// <para>NOT tenant-scoped — a system-wide internal sweep with no actor context. Finds no rows when there are no stale
/// Pending supervisor decisions. Returns the count expired for log surfacing + the recurring-job result.</para>
/// </summary>

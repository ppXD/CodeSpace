...
public sealed class ListProjectVariablesQueryHandler : IRequestHandler<ListProjectVariablesQuery, IReadOnlyList<VariableSummary>>
{
    ...
    public async Task<IReadOnlyList<VariableSummary>> Handle(ListProjectVariablesQuery request, CancellationToken cancellationToken)
    {
        return await _service.ListAsync(VariableScope.Project, request.ProjectId, _currentTeam.Id!.Value, cancellationToken).ConfigureAwait(false);
    }
}

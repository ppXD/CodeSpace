...
/// team) reads the run's overall status through the same request-scoped body-blind identity bundle. A foreign / absent run → the
...
public sealed class GetRunTimelineQueryHandler : IRequestHandler<GetRunTimelineQuery, RunTimelineResponse?>
{
    ...
    public GetRunTimelineQueryHandler(IRunTimelineProjector projector, IWorkflowRunObservationIdentityBundle identity, ICurrentTeam currentTeam)
    {

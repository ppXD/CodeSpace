...
public sealed class SetProjectVariableCommandHandler : IRequestHandler<SetProjectVariableCommand, Unit>
{
    ...
    public async Task<Unit> Handle(SetProjectVariableCommand request, CancellationToken cancellationToken)
    {
        ...
        await _service.SetAsync(VariableScope.Project, request.ProjectId, teamId, request.Name, request.ValueType, request.Value, request.Description, _currentUser.Id!.Value, cancellationToken).ConfigureAwait(false);
        return Unit.Value;
    }
}

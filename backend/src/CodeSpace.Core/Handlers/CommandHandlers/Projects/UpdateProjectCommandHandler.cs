...
public sealed class UpdateProjectCommandHandler : IRequestHandler<UpdateProjectCommand, Unit>
{
    ...
    {
        await _service.UpdateAsync(_currentTeam.Id!.Value, request.ProjectId, request.Name, request.Description, _currentUser.Id!.Value, cancellationToken).ConfigureAwait(false);
        return Unit.Value;
    }
}

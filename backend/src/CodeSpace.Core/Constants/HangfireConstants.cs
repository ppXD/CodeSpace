...
public static class HangfireConstants
{
    ...
    /// <summary>The AGENT queue: the long-running <c>IAgentRunExecutor</c> jobs (an agent.code run can hold a worker for minutes while a codex/claude child runs). Isolated onto its own worker pool so it can't starve the control plane. Pinned by a test (Rule 8).</summary>

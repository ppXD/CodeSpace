...
public class RunScorecard : IEntity<Guid>, IAuditable
{
    ...
    /// <summary>The lesson A/B arm the run ran under (<c>LessonArms</c>: injected / withheld / none), read from its prompt receipts across supervisor, planner, and agent-runtime lanes. Null only for legacy or non-agent runs outside the experiment, which is NOT the same claim as the <c>none</c> control.</summary>

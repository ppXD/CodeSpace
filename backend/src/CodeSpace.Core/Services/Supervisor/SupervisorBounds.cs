...
/// one more decision — depth / no-progress) and <see cref="PostDecision"/> (the decider's chosen
/// decision would breach a per-decision bound — spawn fan-out / total-spawn cap / cost cap). Each force-STOP is fail-closed:
...
public static class SupervisorBounds
{
    ...
    /// re-derives the same forced stop: depth (a recursive supervisor nested too deep — checked at turn 0), then the
    /// best-effort no-progress guard. There is NO round / decision-count budget — a supervised run LOOPS UNTIL DONE,
    /// bounded instead by no-progress (a run making no fresh progress stops), the total-spawn cap + cost cap (PostDecision),
    /// and the model's own success stop. Returns the terminal reason, or null to proceed.
    /// </summary>

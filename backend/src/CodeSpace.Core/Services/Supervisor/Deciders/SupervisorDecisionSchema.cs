                    ...
                    "properties": {
                      ...
                      "acceptance": {
                        ...
                        "properties": {
                          ...
                          "protectedPaths": { "type": "array", "items": { "type": "string" }, "description": "Repo-relative paths (git pathspecs) whose bytes belong to the ORACLE, not the worker — a JUDGE or FIXTURE file this subtask must not touch. The server restores these from the task's base before grading, so a worker's edits to them are VOID (and recorded as tamper evidence). NEVER name a file this subtask is expected to modify, and never the file the command EXECUTES as its subject ('sh solution.sh 7 5' when the subtask must edit solution.sh): the server grades the candidate's own copy of the file under test, so naming it would only void the very work you asked for and no retry could pass. Most subtasks need no protectedPaths at all — omit it unless a separate judge/fixture file exists that the worker must leave alone." },
              ...
              "properties": {
                ...
                "acceptance": {
                  ...
                  "properties": {
                    ...
                    "rubric": { "type": "object", "additionalProperties": false, "properties": { "criteria": { "type": "array", "minItems": 1, "items": { "type": "object", "additionalProperties": false, "properties": { "id": { "type": "string" }, "requirement": { "type": "string" }, "weight": { "type": "number" } }, "required": ["id", "requirement"] } }, "threshold": { "type": "number" } }, "required": ["criteria"], "description": "REQUIRED for kind=LlmJudge." },
                    "schema": { "type": "object", "additionalProperties": true, "description": "REQUIRED for kind=ArtifactSchema." },
                    "protectedPaths": { "type": "array", "items": { "type": "string" }, "description": "Repo-relative paths whose bytes belong to the ORACLE, not the worker — judge/fixture files the worker must leave alone. Never name a file the subtask must modify, or the file this command EXECUTES as its subject: the server grades the candidate's own copy of the file under test." },

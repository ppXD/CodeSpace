...
internal static class TaskSpecAcceptanceAssessment
{
    ...
    {
        var resolution = TaskSpecSourceArgv.Resolve(compilation, context);
        var argv = resolution.Argv.ToArray();
        if (compilation.AcceptanceArgvSource is null && (argv.Length == 0 || argv.All(string.IsNullOrWhiteSpace))) return null;
        ...
        var validCommand = argv.Length is > 0 and <= 256 && !string.IsNullOrWhiteSpace(argv[0]) && argv.All(t => t is not null && !t.Contains('\0'));
        ...
        if (resolution.Error is { } sourceError) reason = sourceError;
        else if (resolution.Source is { } boundSource && !evidence.Any(e => e.SourceId == boundSource.Id)) reason = "The semantic review did not cite the literal argv source reference; support remains unknown.";
        else if (!validCommand) reason = "The proposed argv is malformed; it cannot be adopted as an executable check.";

    ...
    public static readonly JsonElement ResponseSchema = JsonDocument.Parse("""
        {
          ...
          "properties": {
            ...
            "acceptanceArgvSource": { "type": ["object", "null"], "additionalProperties": false, "properties": { "sourceId": { "type": "string" }, "quote": { "type": "string", "description": "Exact complete JSON argv array excerpt from the selected source, including every empty argument. The server verifies the excerpt and decodes it; do not regenerate its tokens in acceptanceChecks." }, "encoding": { "type": "string", "enum": ["json-argv"] } }, "required": ["sourceId", "quote", "encoding"], "description": "Optional literal reference when proposing a command from an exact JSON argv array in a supplied source. Omit or return null when no executable check is being proposed; do not fill an empty reference object. Leave acceptanceChecks empty when using this reference. This verifies token fidelity only; an independent semantic review still determines whether the command is requested and relevant." },

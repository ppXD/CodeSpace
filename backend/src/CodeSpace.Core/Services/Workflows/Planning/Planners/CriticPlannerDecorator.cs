...
public sealed class CriticPlannerDecorator : IWorkflowPlanner
{
    ...
    public async Task<PlannedWorkflow> PlanAsync(WorkflowPlanRequest request, CancellationToken cancellationToken)
    {
        ...
        if (verdict.Failed)
            verdict = await _critic.ReviewAsync(
                new CriticRequest
                {
                    Mode = request.Review, ArtifactKind = CriticArtifactKinds.WorkflowPlan, Artifact = Render(plan), Goal = request.TaskText,
                    ProducerModelRowId = request.BrainModelId,
                    ProducerModel = new ReviewModelIdentity { ModelCredentialModelId = request.BrainModelId, ConfiguredModel = plan.AuthoredByModel, ObservedModel = plan.AuthoredByObservedModel },
                },
    ...
    {
        // Both agent and model verdicts reach this header. Name an observed model when available, without claiming
        // independence from a producer whose identity has not been compared here.
        var reviewer = string.IsNullOrWhiteSpace(verdict.ReviewerModel) ? "Review" : $"Review on {verdict.ReviewerModel}";

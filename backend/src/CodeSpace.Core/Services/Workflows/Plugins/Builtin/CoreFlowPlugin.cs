...
/// the universal Terminal, the branching primitives (logic.if / logic.merge), and the
/// iteration primitive (flow.iterate). These can't be removed without crippling the
/// engine itself, so they ship in their own module that's always loaded.
///
...
/// deployment could load ONLY this plugin and still run validation + simple flows.
/// </summary>

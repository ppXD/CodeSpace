...
public sealed class StorageProfileProbeService : IStorageProfileProbeService
{
    ...
    public async Task<StorageProfileProbeResult> ProbeAsync(StorageProfileProbeRequest request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result(new ProbeResultContext(request.ProfileId, request.ProfileRevision, null, request.VerifyWriteAccess, stopwatch, Provisions(request)), StorageProfileProbeStatusValue.Cancelled, Failure(StorageProfileProbeFailureStageValue.Cancellation, StorageProfileProbeFailureCodeValue.CancelledProfileResolution, true));
        }
        catch (Exception exception) when (IsRecoverable(exception))
        {
            return Result(new ProbeResultContext(request.ProfileId, request.ProfileRevision, null, request.VerifyWriteAccess, stopwatch, Provisions(request)), StorageProfileProbeStatusValue.Unavailable, Failure(StorageProfileProbeFailureStageValue.Profile, StorageProfileProbeFailureCodeValue.ProfileResolutionFailed, true));
        }

        if (target == null)
            return Result(new ProbeResultContext(request.ProfileId, request.ProfileRevision, null, request.VerifyWriteAccess, stopwatch, Provisions(request)), StorageProfileProbeStatusValue.Unavailable, Failure(StorageProfileProbeFailureStageValue.Profile, StorageProfileProbeFailureCodeValue.ProfileMissing, false));

...
/// <summary>
/// Thrown by <see cref="MissingRequiredInputValidator"/> when the engine is running in
/// <see cref="Hardening.EnforcementMode.Strict"/> and a workflow definition declares an
/// input as <c>Required = true</c> that neither the manual-run / webhook trigger payload
...
/// blank LLM prompts, etc.). Strict mode throws this so the run lands in

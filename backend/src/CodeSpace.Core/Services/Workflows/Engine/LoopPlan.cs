...
/// </summary>
public readonly record struct LoopPlan(int MaxIterations, TimeSpan WallClock, int NodeBudget)
{
    ...
    public static LoopPlan From(LoopConfig config) =>
        new(Math.Clamp(config.MaxIterations <= 0 ? 1 : config.MaxIterations, 1, MaxIterationsCeiling), WallClockBudget, NodeExecutionBudget);
}

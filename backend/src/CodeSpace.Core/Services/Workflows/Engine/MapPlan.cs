...
/// </summary>
public readonly record struct MapPlan(MapErrorHandling ErrorHandling, string ResultKey, int? MaxParallelism, decimal? MaxCostUsd)
{
    ...
    public static MapPlan From(MapConfig config) =>
        new(ParseErrorHandling(config.ErrorHandling), NormalizeResultKey(config.ResultKey), config.MaxParallelism, config.MaxCostUsd is > 0 ? config.MaxCostUsd : null);

...
public sealed class WorkflowEngine : IWorkflowEngine, IScopedDependency
{
    ...
    {
        // Owns its transaction deliberately (not ScopedTransaction.OwnOrJoinAsync): the ceremonies below must run
        // AFTER this terminal is visible, which only an owned commit here guarantees. Joined, "outside the
        // transaction" would become "inside the caller's", and a caller's rollback would unpublish a terminal whose
        // ceremonies had already fired. The engine walks on its own scope, so nothing is ambient here.
    ...
    {
        // Owns its transaction deliberately (not ScopedTransaction.OwnOrJoinAsync): CancellationToken.None says this
    ...
    private async Task MarkBootstrapFailureAsync(Guid runId, DateTimeOffset engineStartedAt, Exception failure)
    {
        ...
        // Owns its transaction deliberately (not ScopedTransaction.OwnOrJoinAsync), for the same reason
        // EnsureRunCancelledAsync does: a bootstrap terminal written under CancellationToken.None must land on its
    ...
    private async Task CompleteNodeWithOffloadAsync(NodeCompletionPersistence completion, CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Owns its transaction deliberately (not ScopedTransaction.OwnOrJoinAsync): the catch below re-writes the
            // record through a FRESH scope, which is only correct because disposing this one discards the first
            // write. Joined, that discard would be a no-op and the node would settle twice.
            await using var transaction = await _db.Database.BeginTransactionAsync(cancellationToken).ConfigureAwait(false);

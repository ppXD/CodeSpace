...
public sealed class RunFromSnapshotStarter : IRunFromSnapshotStarter, IScopedDependency
{
    ...
    public async Task<Guid> StartFromSnapshotAsync(WorkflowDefinition definition, Guid teamId, Guid actorUserId, string? launchPayloadJson, IReadOnlyList<Guid>? scopeRepositoryIds, string? projectionKind, SessionAssignment? session, CancellationToken cancellationToken)
    {
        ...
        var runId = await StageAsync(teamId, actorUserId, definitionJson, definitionHash, payloadJson, WorkflowRunSourceTypes.Snapshot, parentRunId: null, rootRunId: null, rerunFromNodeId: null, causationRequestId: null, repositoryIds, projectIds, projectionKind, session, idempotencyKey: null, cancellationToken).ConfigureAwait(false);
    ...
    /// </summary>
    public async Task<Guid> StageReplayFromSnapshotAsync(string definitionJson, string definitionHash, Guid teamId, Guid actorUserId, string payloadJson, string sourceType, Guid parentRunId, Guid rootRunId, string? rerunFromNodeId, Guid causationRequestId, IReadOnlyList<Guid> scopeRepositoryIds, IReadOnlyList<Guid> scopeProjectIds, string? projectionKind, SessionAssignment? session, string? idempotencyKey, CancellationToken cancellationToken)
    {
        ...
        var runId = await StageAsync(teamId, actorUserId, definitionJson, definitionHash, NormalizePayload(payloadJson), sourceType, parentRunId, rootRunId, rerunFromNodeId, causationRequestId, scopeRepositoryIds, scopeProjectIds, projectionKind, session, idempotencyKey, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<Guid> StageAsync(Guid teamId, Guid actorUserId, string definitionJson, string definitionHash, string payloadJson, string sourceType, Guid? parentRunId, Guid? rootRunId, string? rerunFromNodeId, Guid? causationRequestId, IReadOnlyList<Guid> scopeRepositoryIds, IReadOnlyList<Guid> scopeProjectIds, string? projectionKind, SessionAssignment? session, string? idempotencyKey, CancellationToken cancellationToken)
    {
        ...
        _db.WorkflowRun.Add(new WorkflowRun
        {
            ...
            CompletionEnforcementMode = Completion.CompletionPolicy.StampModeFor(DefinitionCompletionMode.Read(definitionJson), mode, _modes.Resolve(mode)).ToString(),

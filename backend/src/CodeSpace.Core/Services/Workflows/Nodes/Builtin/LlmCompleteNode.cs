        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                "provider": { "type": "string", "enum": ["Anthropic", "OpenAI", "Custom"], "default": "Anthropic", "x-control": "radioCards", "x-enumLabels": { "Anthropic": "Anthropic (Claude)", "OpenAI": "OpenAI", "Custom": "Custom gateway" }, "x-optionConsequence": { "Anthropic": "Anthropic's Claude models — via an Anthropic model credential.", "OpenAI": "The OpenAI API, or an OpenAI-tagged compatible gateway — via an OpenAI model credential.", "Custom": "Your own OpenAI-compatible endpoint (LiteLLM / vLLM / OpenRouter…) — via a Custom-gateway model credential." }, "description": "Which wire to call — match it to how your model credential is set up. The Model list below shows what's enabled under the chosen provider." },
                "model": { "type": "string", "x-selector": "poolModel", "title": "Model", "description": "Optional. Pins ONE model from your team's pool for the provider above — pick it from the list. Leave empty to let the pool pick its recommended model, or switch to Expression to bind a {{ref}}." },

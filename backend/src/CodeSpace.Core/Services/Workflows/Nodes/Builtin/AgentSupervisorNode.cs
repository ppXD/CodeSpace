...
public sealed class AgentSupervisorNode : INodeRuntime
{
    ...
    public NodeManifest Manifest { get; } = new()
    {
        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              "type": "object",
              "x-sections": ["Goal", "Agents & repos", "Guardrails", "Review & approval"],
              "properties": {
                "goal": { "x-group": "Goal", "type": "string", "title": "Goal", "x-long": true, "description": "What the supervisor should accomplish across the run — folded into every decision it makes. e.g. \"Fix the failing auth tests and open a PR.\"" },
                "synthesisPromptBudgetChars": { "x-group": "Guardrails", "type": "integer", "minimum": 2000, "maximum": 1000000, "title": "Synthesis input limit", "x-advanced": true, "description": "Maximum characters handed to the optional merge-synthesis model. Large multi-agent diffs are fairly excerpted with explicit coverage; deterministic integration still reads the full artifacts." },
                "conversationId": { "x-group": "Review & approval", "type": "string", "format": "uuid", "x-selector": "conversation", "description": "Conversation the supervisor posts its ask_human + approval questions into (and parks for the answer). Leave empty to disable mid-loop human questions + approval gating." },
                "maxParallelism": { "x-group": "Agents & repos", "type": "integer", "minimum": 1, "maximum": 20, "title": "Max agents at once", "description": "Max agents one spawn decision may fan out at once. Defaults to 20 (the ceiling)." },
                "maxTotalSpawns": { "x-group": "Guardrails", "type": "integer", "minimum": 1, "maximum": 1000, "title": "Max agents total", "x-advanced": true, "description": "Max agents the whole run may spawn in total before it force-stops. Defaults to 50." },
                "maxCostUsd": { "x-group": "Guardrails", "type": "number", "minimum": 0, "title": "Cost limit (USD)", "description": "Optional USD budget for the run's agent token spend. Leave empty for no cap (the total-spawn cap still bounds it). Spend above the budget force-stops the next spawn." },
                "maxNoProgressDecisions": { "x-group": "Guardrails", "type": "integer", "minimum": 1, "maximum": 30, "title": "Give up after N stalled turns", "x-advanced": true, "description": "Force-stop after this many consecutive decisions produce no new agent result. Defaults to 8." },
                "approvalPolicy": { "x-group": "Review & approval", "type": "string", "enum": ["none", "spawns"], "title": "Approve before spawning", "x-control": "radioCards", "x-enumLabels": { "none": "Autonomous", "spawns": "Ask before each spawn" }, "x-optionConsequence": { "none": "The supervisor spawns and retries agents immediately, without waiting for approval.", "spawns": "No agent is created until a human approves each spawn or retry in the conversation." }, "description": "Whether a human must approve every spawn/retry before any agent is created. Autonomous (default) runs without gating." },
                "allowedAgents": { "x-group": "Guardrails", "type": "array", "items": { "type": "string" }, "x-advanced": true, "description": "Effective harness kinds the supervisor may spawn (for example codex-cli or claude-code). The supervisor is only offered these, and an agent it dispatches can neither be authored onto nor repaired onto a kind outside the list; a default outside it is narrowed to the list rather than failing the run. Empty = all registered harnesses." },
                "allowedTools": { "x-group": "Guardrails", "type": "array", "items": { "type": "string" }, "title": "Tool allow-list", "x-advanced": true, "description": "Tools every spawned agent is restricted to (e.g. Read, Grep, Bash). Empty = the harness default. Added to (not replacing) a persona's tools." },
                "supervisorModelId": { "x-group": "Agents & repos", "type": "string", "format": "uuid", "title": "Lead model", "x-selector": "credentialedModel", "description": "REQUIRED. The model the supervisor's own decision-making runs on — pick one from the team's credentialed models (distinct from the agents it spawns). The run fails closed if it's missing." },
                "decisionReviewMode": { "x-group": "Review & approval", "type": "integer", "enum": [0, 1, 2], "title": "Review every decision", "x-control": "radioCards", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "x-optionConsequence": { "0": "Turns run without a decision critic — the supervisor's own call is used as-is.", "1": "A reviewer checks each decision; an objection triggers one redo, and a still-rejected decision pauses the run for your approval instead of running.", "2": "A reviewer critiques each decision; if it flags a material issue the supervisor revises once before acting — the critic never blocks or pauses the run." }, "description": "An independent critic reviews each supervisor decision before its side effect. Off (default) = no review." },
                "planReviewMode": { "x-group": "Review & approval", "type": "integer", "enum": [0, 1, 2], "default": 0, "title": "Review the plan", "x-control": "radioCards", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "x-optionConsequence": { "0": "Plans get no dedicated review — they're only reviewed if \"Review every decision\" is on, using that setting.", "1": "Each plan is reviewed before any agent runs; a rejected plan is revised and re-checked once, and if still rejected the run pauses for your approval.", "2": "Each plan is critiqued; if the reviewer flags a material issue it's automatically revised once before agents run — no pause for you." }, "x-advanced": true, "description": "A critic scoped to ONLY the supervisor's plan decisions. When unset, plan decisions fall under \"Review every decision\"." },
                "reviewerModelId": { "x-group": "Review & approval", "type": "string", "format": "uuid", "title": "Reviewer model", "x-selector": "credentialedModel", "x-advanced": true, "description": "The model the critic runs on (ideally distinct from the lead). Leave empty to auto-pick the team's strongest eligible model. Only used when a review mode is on." },
                "allowedModelIds": { "x-group": "Agents & repos", "type": "array", "items": { "type": "string", "format": "uuid" }, "title": "Allowed models", "x-selector": "credentialedModel", "description": "The models dispatched agents may use — a multi-select. Every dispatched agent's model must be one of these. Empty = all the team's credentialed models." },
                "allowedAgentDefinitionIds": { "x-group": "Agents & repos", "type": "array", "items": { "type": "string", "format": "uuid" }, "title": "Allowed agents", "x-selector": "agent", "description": "The personas dispatched agents may embody — a multi-select. Every dispatched agent's persona must be one of these. Empty = all the team's personas." },
                "acceptanceChecks": { "x-group": "Guardrails", "type": "array", "items": { "type": "string" }, "description": "The operator acceptance FLOOR — an argv (e.g. [\"sh\",\"check.sh\"]) run against the run's reviewable head at the terminal stop. Mandatory when set: a non-zero exit fails the stop and withholds the reviewable branch, so success can't be declared on an unverified head." },
                "acceptanceCriteria": { "x-group": "Goal", "type": "array", "items": { "type": "string" }, "description": "Free-text acceptance CRITERIA the operator wants met (e.g. \"tests pass\", \"PR opened\") — rendered into the decider prompt as the definition of done the supervisor targets. NOT executed (unlike acceptanceChecks, which is an argv floor)." },
                "requirePlanConfirmation": { "x-group": "Review & approval", "type": "boolean", "title": "Pause for plan approval", "description": "When on, every authored plan version parks the run on a confirmation card before any agent is created — approve to release, or reply with revision feedback. Off (default) = fully autonomous planning." },
                "deliverySpec": { "x-group": "Guardrails",
                  "type": "object",
                ...
                "agentProfile": { "x-group": "Agents & repos",
                  "type": "object",
                  "additionalProperties": false,
                    ...
                    "relatedRepositories": {
                      ...
                      "items": {
                        ...
                        "properties": {
                          ...
                          "access": { "type": "string", "enum": ["read", "write"], "title": "Access", "x-enumLabels": { "read": "Read-only", "write": "Read + write" }, "description": "read = context-only (default); write = the agent may edit + branch it." }
                        },
                        "required": ["repositoryId"]
                      }
                    },
                    "harness": { "type": "string", "title": "Harness", "x-selector": "harness", "description": "Which coding-agent CLI each spawned agent runs (e.g. Codex, Claude Code). Defaults to codex-cli." },
                    "model": { "type": "string", "title": "Model", "description": "Model id within the harness's catalog. Leave empty to use the persona's model, or the harness default." },
                    "agentDefinitionId": { "type": "string", "format": "uuid", "title": "Agent persona", "x-selector": "agent", "description": "Agent persona each spawned agent embodies — its system prompt + model + tools become the defaults. Leave empty to configure inline." },
                    "modelCredentialId": { "type": "string", "format": "uuid", "title": "Model credential", "x-selector": "modelCredential", "x-advanced": true, "description": "Model credential each spawned agent authenticates with. Leave empty for the persona's or the team/operator default." },
                    "runnerKind": { "type": "string", "x-advanced": true, "description": "Sandbox runner each spawned agent executes on (e.g. \"local\"). Empty → the deployment default, set by the Agents:DefaultRunnerKind configuration key (Agents__DefaultRunnerKind in the environment); \"local\" when that is unset." },
                    "timeoutSeconds": { "type": "integer", "title": "Timeout (seconds)", "x-advanced": true, "description": "Wall-clock cap for each spawned agent, in seconds. Positive caps the run; 0 = no wall-clock (bounded only by the stall watchdog + cost cap); leave empty for the bounded 1h default." },
                    "enableMcp": { "type": "boolean", "title": "Enable MCP tools", "x-advanced": true, "description": "Open the MCP tool-fabric endpoint for each spawned agent. Leave unset to defer to the deployment default." },
                    "autonomyLevel": { "type": "string", "enum": ["Confined", "Standard", "Trusted", "Unleashed"], "title": "Autonomy", "x-control": "radioCards", "x-enumLabels": { "Confined": "Read-only, no network", "Standard": "Workspace write, no network", "Trusted": "Adds network access", "Unleashed": "Unattended, no approvals" }, "x-optionConsequence": { "Confined": "Every spawned agent reads and analyzes only — no file changes, no network, and destructive tools are refused.", "Standard": "Each spawned agent edits files in its workspace with no network; risky tool calls pause for human approval.", "Trusted": "Each spawned agent gets the same workspace writes as Standard plus outbound network; risky tool calls still pause for approval.", "Unleashed": "Each spawned agent gets the same writes and network as Trusted, but its risky tool calls run without asking — except irreversible or dangerous ones (a PR merge, rm -rf, sudo…), which still require approval." }, "description": "How much each spawned agent may do (write scope + network). Standard is the default." }
                  }
                }
              }
            }
        ...
        OutputSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                ...
                "reason":           { "type": "string" },
                "summary":          { "type": "string", "description": "The final model-authored synthesis of the multi-agent result, or the terminal reason when no synthesis was produced." },
                "turns":            { "type": "integer" },
    ...
    internal static NodeResult Finish(Microsoft.Extensions.Logging.ILogger logger, SupervisorTurnResult result, Guid? repositoryId = null)
    {
        ...
        var outputs = new Dictionary<string, JsonElement>
        {
            ...
            ["summary"] = JsonSerializer.SerializeToElement(result.StopClassification?.DisplayText ?? result.TerminalReason ?? ""),

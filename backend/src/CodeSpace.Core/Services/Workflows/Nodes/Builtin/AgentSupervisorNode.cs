              ...
              "properties": {
                ...
                "agentProfile": { "x-group": "Agents & repos",
                  ...
                  "properties": {
                    ...
                    "autonomyLevel": { "type": "string", "enum": ["Confined", "Standard", "Trusted", "Unleashed"], "title": "Autonomy", "x-control": "radioCards", "x-enumLabels": { "Confined": "Read-only, no network", "Standard": "Workspace write, no network", "Trusted": "Adds network access", "Unleashed": "Unattended, no approvals" }, "x-optionConsequence": { "Confined": "Every spawned agent reads and analyzes only — no file changes, no network, and destructive tools are refused.", "Standard": "Each spawned agent edits files in its workspace with no network; risky tool calls pause for human approval.", "Trusted": "Each spawned agent gets the same workspace writes as Standard plus outbound network; risky tool calls still pause for approval.", "Unleashed": "Each spawned agent gets the same writes and network as Trusted, but its risky tool calls run without asking — except irreversible ones like PR merges, which still require approval." }, "description": "How much each spawned agent may do (write scope + network). Standard is the default." }
                  }
                }
              }
            }

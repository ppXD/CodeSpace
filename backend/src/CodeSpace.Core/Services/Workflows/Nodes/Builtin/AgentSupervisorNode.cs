        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                ...
                "approvalPolicy": { "x-group": "Review & approval", "type": "string", "enum": ["none", "spawns"], "title": "Approve before spawning", "x-enumLabels": { "none": "Autonomous", "spawns": "Ask before each spawn" }, "description": "Whether a human must approve every spawn/retry before any agent is created. Autonomous (default) runs without gating." },
                ...
                "decisionReviewMode": { "x-group": "Review & approval", "type": "integer", "enum": [0, 1, 2], "title": "Review every decision", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "description": "An independent critic reviews each decision before its side effect. Gate = flag concerns; Improve = one bounded re-decide against the critique. Off (default) = no review." },
                "planReviewMode": { "x-group": "Review & approval", "type": "integer", "enum": [0, 1, 2], "default": 0, "title": "Review the plan", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "x-advanced": true, "description": "A critic scoped to ONLY the supervisor's plan decisions. When unset, plan decisions fall under \"Review every decision\"." },
                ...
                "agentProfile": { "x-group": "Agents & repos",
                  ...
                  "properties": {
                    ...
                    "autonomyLevel": { "type": "string", "enum": ["Confined", "Standard", "Trusted", "Unleashed"], "title": "Autonomy", "x-enumLabels": { "Confined": "Confined — read-only", "Standard": "Standard — workspace write", "Trusted": "Trusted — + network", "Unleashed": "Unleashed — full" }, "description": "How much each spawned agent may do (write scope + network). Standard is the default." }
                  }
                }
              }
            }

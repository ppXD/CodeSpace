        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                ...
                "supervisorModelId": { "type": "string", "format": "uuid", "x-selector": "credentialedModel", "description": "REQUIRED. The credentialed model the supervisor's own decision-making brain runs on — pick one from a credential's model list (distinct from the agents it spawns). Must be a structured-capable model; the run fails closed if absent or unresolvable." },

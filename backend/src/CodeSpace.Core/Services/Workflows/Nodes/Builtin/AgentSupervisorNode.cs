        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              "type": "object",
              "properties": {
                "goal": { "type": "string", "title": "Goal", "x-long": true, "description": "What the supervisor should accomplish across the run — folded into every decision it makes. e.g. \"Fix the failing auth tests and open a PR.\"" },
                ...
                "maxParallelism": { "type": "integer", "minimum": 1, "maximum": 20, "title": "Max agents at once", "description": "Max agents one spawn decision may fan out at once. Defaults to 20 (the ceiling)." },
                "maxTotalSpawns": { "type": "integer", "minimum": 1, "maximum": 1000, "title": "Max agents total", "x-advanced": true, "description": "Max agents the whole run may spawn in total before it force-stops. Defaults to 50." },
                "maxCostUsd": { "type": "number", "minimum": 0, "title": "Cost limit (USD)", "description": "Optional USD budget for the run's agent token spend. Leave empty for no cap (the total-spawn cap still bounds it). Spend above the budget force-stops the next spawn." },
                "maxNoProgressDecisions": { "type": "integer", "minimum": 1, "maximum": 30, "title": "Give up after N stalled turns", "x-advanced": true, "description": "Force-stop after this many consecutive decisions produce no new agent result. Defaults to 8." },
                "approvalPolicy": { "type": "string", "enum": ["none", "spawns"], "title": "Approve before spawning", "x-enumLabels": { "none": "Autonomous", "spawns": "Ask before each spawn" }, "description": "Whether a human must approve every spawn/retry before any agent is created. Autonomous (default) runs without gating." },
                "allowedAgents": { "type": "array", "items": { "type": "string" }, "x-advanced": true, "description": "Reserved — harness/agent kinds the supervisor may spawn. Stored; enforcement is a follow-up." },
                "allowedTools": { "type": "array", "items": { "type": "string" }, "title": "Tool allow-list", "x-advanced": true, "description": "Tools every spawned agent is restricted to (e.g. Read, Grep, Bash). Empty = the harness default. Added to (not replacing) a persona's tools." },
                "supervisorModelId": { "type": "string", "format": "uuid", "title": "Lead model", "x-selector": "credentialedModel", "description": "REQUIRED. The model the supervisor's own decision-making runs on — pick one from the team's credentialed models (distinct from the agents it spawns). The run fails closed if it's missing." },
                "decisionReviewMode": { "type": "integer", "enum": [0, 1, 2], "title": "Review every decision", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "description": "An independent critic reviews each decision before its side effect. Gate = flag concerns; Improve = one bounded re-decide against the critique. Off (default) = no review." },
                "planReviewMode": { "type": "integer", "enum": [0, 1, 2], "default": 0, "title": "Review the plan", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "x-advanced": true, "description": "A critic scoped to ONLY the supervisor's plan decisions. When unset, plan decisions fall under \"Review every decision\"." },
                "reviewerModelId": { "type": "string", "format": "uuid", "title": "Reviewer model", "x-selector": "credentialedModel", "x-advanced": true, "description": "The model the critic runs on (ideally distinct from the lead). Leave empty to auto-pick the team's strongest eligible model. Only used when a review mode is on." },
                "allowedModelIds": { "type": "array", "items": { "type": "string", "format": "uuid" }, "title": "Allowed models", "x-selector": "credentialedModel", "description": "The models dispatched agents may use — a multi-select. Every dispatched agent's model must be one of these. Empty = all the team's credentialed models." },
                "allowedAgentDefinitionIds": { "type": "array", "items": { "type": "string", "format": "uuid" }, "title": "Allowed agents", "x-selector": "agent", "description": "The personas dispatched agents may embody — a multi-select. Every dispatched agent's persona must be one of these. Empty = all the team's personas." },
                ...
                "requirePlanConfirmation": { "type": "boolean", "title": "Pause for plan approval", "description": "When on, every authored plan version parks the run on a confirmation card before any agent is created — approve to release, or reply with revision feedback. Off (default) = fully autonomous planning." },
                "deliverySpec": {
                  "type": "object",

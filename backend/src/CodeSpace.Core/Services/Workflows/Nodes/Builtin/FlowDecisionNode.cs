        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                ...
                "answerSchema":      { "type": "object", "description": "Optional JSON Schema describing the expected answer shape (recorded for downstream use; not enforced on the answer)." },
                ...
                "policy":            { "type": "string", "enum": ["auto_allowed","supervisor_first","human_required"], "default": "human_required", "title": "Who answers", "x-enumLabels": { "auto_allowed": "Auto-answer allowed", "supervisor_first": "Supervisor may answer", "human_required": "Human required" }, "description": "The autonomy this decision permits. A standalone decision node still waits for a person (or the timeout default below) — the automated options take effect only when an agent or supervisor drives the decision." },

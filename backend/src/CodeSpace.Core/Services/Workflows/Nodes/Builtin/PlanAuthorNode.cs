...
public sealed class PlanAuthorNode : INodeRuntime
{
    ...
    public NodeManifest Manifest { get; } = new()
    {
        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              "type": "object",
              "x-sections": ["Planning", "Review"],
              "x-intent": "Turn the goal into a durable, checkable plan — reasoning on {plannerModelId}.",
              "x-intentPlaceholders": { "plannerModelId": "the team's strongest model" },
              "properties": {
                "plannerModelId": { "x-group": "Planning", "type": "string", "format": "uuid", "title": "Planner model", "x-selector": "credentialedModel", "description": "The model the planner reasons on. Leave empty to auto-pick the team's strongest eligible model." },
                "reviewMode": { "x-group": "Review", "type": "integer", "enum": [0, 1, 2], "default": 0, "title": "Review the plan", "x-control": "radioCards", "x-enumLabels": { "0": "Off", "1": "Gate", "2": "Improve" }, "x-optionConsequence": { "0": "No independent review — the plan is used as authored.", "1": "A reviewer flags concerns onto the plan's risks, but doesn't change the plan.", "2": "A reviewer makes one bounded revision of the plan against its own critique." }, "description": "An independent reviewer over the produced plan. Gate flags concerns onto the plan's risks; Improve makes one bounded revision against the critique." },
                "reviewerModelId": { "x-group": "Review", "type": "string", "format": "uuid", "title": "Reviewer model", "x-selector": "credentialedModel", "x-advanced": true, "description": "The model the reviewer runs on (ideally distinct from the planner). Leave empty to auto-pick. Only used when a review mode is on." },
                "flatPlan": { "x-group": "Planning", "type": "boolean", "default": false, "title": "Independent subtasks only", "x-advanced": true, "description": "Constrain the plan to independent subtasks (no dependsOn) — set by parallel fan-out projections (flow.map runs every item concurrently). Authored dependencies are stripped as a fail-safe (logged)." },
                "reviewerAgent": { "x-group": "Review", "type": "boolean", "default": false, "title": "Review against the real repo", "x-advanced": true, "description": "Review the plan with a real independent agent that clones the repository below and verifies it against the actual code, instead of only the in-process model critic. Falls back to the model critic when the agent can't produce a verdict. Only used when a review mode is on AND a repository is set." },

        ...
        ConfigSchema = SchemaBuilder.Parse("""
            {
              ...
              "properties": {
                ...
                "resolve": {
                  ...
                  "properties": {
                    "mode": { "type": "string", "enum": ["first","quorum"], "default": "first", "x-control": "radioCards", "x-enumLabels": { "first": "First response wins", "quorum": "Quorum — N of the same" }, "x-optionConsequence": { "first": "The first person to respond decides — their answer resolves the wait.", "quorum": "Waits until the required number of different people pick the SAME option (set below) before resolving." }, "description": "A button marked `vetoes` always decides on one click, regardless of this." },

...
public static class CgroupResourceLimit
{
    ...
    /// <summary>The configured delegated cgroup root (<c>Sandbox:CgroupRoot</c>), or null when the operator has not delegated a subtree — in which case the durable launch applies no cgroup cap.</summary>
    public static string? CgroupRoot => CodeSpace.Core.Settings.RuntimeSettings.Current.AgentCgroupRoot;

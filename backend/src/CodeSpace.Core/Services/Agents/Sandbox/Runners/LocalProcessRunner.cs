...
/// <c>LocalProcessRunner.Durable.cs</c>). Its siblings <see cref="ISandboxDurableLogSource"/> (bounded,
/// byte-addressed reads of that exact spool for generic Shadow capture) and
/// <see cref="ISandboxDurableDiagnosticSource"/> (the spooled stderr delivered line by line, holding one bounded read
/// pass at a time rather than the stream)
/// read the same spool without widening the runner contract. Enforces <see cref="SandboxSpec.TimeoutSeconds"/> by killing the

...
/// per-string ceiling, its standard input is past what the launch pipe can carry, or its goal is past the harness
/// CLI's own input cap (Codex's, which refuses before any model request). Its own exit scenario rather than a <see cref="NativeLaunchException"/> reason, because the two

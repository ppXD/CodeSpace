...
/// records a COARSE tier in <c>probed_capability_tier</c> — capped at Strong and graded in code (never self-rated).
/// Complete newer evidence replaces stale evidence; incomplete infrastructure-faulted batteries preserve it. A cached
/// FACT driven by a recurring backfill job; never per-launch.
///
...
/// per team. The CONSUMER is the auto pick, where concrete probed evidence wins and absent/Unknown evidence falls back
/// to the brain prior.</para>
/// </summary>
public interface IModelCapabilityProbeService
{
    /// <summary>Probe the team's OPAQUE (capability_tier='Unknown'), stale-or-unprobed pool models and replace the prior observation only when the full battery completes. A no-op when the team has no such model; fail-soft on any error.</summary>

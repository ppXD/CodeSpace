...
/// <see cref="HarnessReductionFold"/> takes from a RECORD. The NAMED once-only facts
/// (<see cref="HarnessReducedStateV1.FirstSessionId"/>, the model-call ids, the last required event type) are taken
/// only from an EXACTLY-GROUNDED projection, and the one projector in production —
/// <see cref="AgentNativeRecordPump"/> — stamps <see cref="SemanticProjectionQuality.Derived"/>
/// unconditionally and correctly, because normalizing a frame onto a shared vocabulary is not transcribing what the
/// harness said. So those fields are null on every real run and this sink resuming them is a property with no
/// production input yet. They start flowing when an exactly-grounded projector lands — the exact-telemetry one reading
/// ModelWire/ToolWire frames — and NOT by relaxing the grounding rule, which is the thing keeping a guess out of a
/// field a warm resume would read as an established fact.</para>
///

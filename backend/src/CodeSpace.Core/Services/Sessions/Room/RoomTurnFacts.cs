...
public sealed record RoomDelivery
{
    ...
    /// <summary>The agents that delivered into THIS repository (P21-8b) — see <see cref="RoomArtifactProducer"/>.</summary>
    public IReadOnlyList<RoomArtifactProducer> Producers { get; init; } = Array.Empty<RoomArtifactProducer>();
}

/// <summary>Turn-facing reduction of one agent's durable log streams. Detail is backend-authored from persisted stream state and integrity evidence. The status vocabulary itself lives in Messages (<see cref="RoomAgentLogStatus"/>) because the Room now puts it on the wire per artifact.</summary>

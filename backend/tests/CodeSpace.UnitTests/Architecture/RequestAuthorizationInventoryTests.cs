...
public class RequestAuthorizationInventoryTests
{
    ...
    private static readonly IReadOnlyDictionary<string, string> SystemSweepByDesign = new Dictionary<string, string>
    {
        ...
        ["ProbeStaleStorageDestinationsCommand"] = "re-probes the destinations Active routes bind writes to and the destinations that still hold stored objects, across every team; a per-team capability would make the deployment-wide question unaskable, and it changes no row a user owns — only the observed health of a destination.",
        ...
        ["ReapAgentRunOrphansCommand"] = "settles the cleanup receipts addressed to THIS host, across every team, by tearing down or observing host-local resources agent runs left behind when they were abandoned from another worker; which kernel objects and directories a machine still holds is a question no team can ask on its own behalf, and it reclaims what a run already finished with rather than changing any run's outcome.",
        ...
        ["ReapExpiredDurableRecordsCommand"] = "reclaims the bytes behind durable records whose retention rule elapsed and that nothing — no column, no qualification pin — still cites, across every team in bounded batches; how long an archive is kept is a deployment-wide policy no team states on its own behalf, and it removes only what both waits and a fail-closed citation probe already cleared.",

...
public class AgentRunExecutorMappingTests
{
    ...
    public void A_stalled_sandbox_maps_to_needs_review_blocked()
    {
        ...
        // NeedsReview(Blocked), not buried as a bare timeout. The harness's folder is not consulted for a non-exit
        // terminal at all — hence the null: the branch reads only the executor's own harness-independent facts.
        var result = AgentRunExecutor.MapSandboxResult(Sandbox(SandboxStatus.Stalled), folder: null!, new AgentRunFacts());
    ...
    {
        var result = AgentRunExecutor.MapSandboxResult(Sandbox(SandboxStatus.TimedOut), folder: null!, new AgentRunFacts());
    ...
    public void A_forced_terminal_still_captures_the_tokens_the_agent_burned(SandboxStatus status)
    {
        ...
        var result = AgentRunExecutor.MapSandboxResult(Sandbox(status), folder: null!, AgentRunFacts.From(events));
    ...
    public void A_forced_terminal_with_no_session_bearing_event_leaves_session_id_null(SandboxStatus status)
    {
        ...
        var result = AgentRunExecutor.MapSandboxResult(Sandbox(status), folder: null!, new AgentRunFacts());

...
public class ClaudeCodeHarnessTests
{
    ...
    public void The_stop_hook_script_resolves_THIS_harnesss_own_config_home_variable()
    {
        ...
        script.ShouldContain($"CFG=\"${ClaudeCodeHarness.ConfigDirEnvVar}\"", Case.Sensitive,
    ...
    [Fact]
    public void The_stop_hook_script_is_marked_executable_and_settings_json_is_not()
    {
        var task = Task() with { Acceptance = new SupervisorAcceptanceSpec { Command = new[] { "sh", "check.sh" } } };
        ...
        files.Single(f => f.RelativePath == InLoopAcceptanceHook.ScriptRelativePath).IsExecutable.ShouldBeTrue(
            "settings.json invokes the script by direct path (\"$CLAUDE_CONFIG_DIR\"/…), which execs the file — without +x the CLI gets exit 126 and the hook silently never runs");
        files.Single(f => f.RelativePath == "settings.json").IsExecutable.ShouldBeFalse();
    }

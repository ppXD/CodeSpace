...
public class SupervisorTurnServiceTests
{
    ...
    public async Task An_unconfirmed_plan_preempts_the_decider_with_the_confirmation_ask_and_flips_the_status()
    {
        ...
        var service = new SupervisorTurnService(new FakeSupervisorDecisionLog(), new AlwaysPlanDecider(), new StubSupervisorActionExecutor(), db: Infrastructure.EmptyTestDb.New(), new FakeAcceptanceGrader(), new FakeDecisionQueue(), new FakeDecisionArbiter(), new FakeDecisionAnswerService(), store, null!, null!, new FakePublishManifestStore(), new FakeSupervisorPublishedBranchResolver(), new NullCompletionComposer(), new AdmitAllBudgetLedger(), new NoLessonsReader(), null!, NullLogger<SupervisorTurnService>.Instance);

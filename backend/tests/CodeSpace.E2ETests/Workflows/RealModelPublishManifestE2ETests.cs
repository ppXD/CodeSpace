...
/// <summary>
/// HIGH fidelity (Rule 12): the live behavioral proof of the publish-manifest arc's I1/I2/I3 invariants — a REAL
/// <c>claude</c> CLI, driven by a REAL model, edits a REAL git clone; the branch is force-pushed over REAL git to a
/// REAL local bare repo (no network — <c>file://</c>, mirroring <see cref="AgentBranchPushFlowTests.BareRemote"/>);
/// and a <see cref="PublishManifest"/> row lands with the pushed branch + the changed file, independently verifiable
/// against the bare remote's own refs. Nothing on the publish path is mocked: the model, the harness process, git
/// clone/commit/push, Postgres, and the artifact store are all real production components.
///
...
public sealed class RealModelPublishManifestE2ETests
{
    ...
    [Fact]
    public async Task A_real_agent_runs_diff_is_captured_pushed_and_recorded_in_the_publish_manifest()
    {
        ...
        await RealModelGate.AssessLiveBestOfNAsync(Provider, async () =>
        {
            ...
            var repositoryId = await SeedBoundRepositoryAsync(teamId, remote.Url, defaultBranch: "main");
            ...
            var task = new AgentTask
            {
                ...
                PushProducedBranch = true,   // per-run opt-in — gates independent of the deployment-wide env flag's state on this lane
                TimeoutSeconds = 180,
            };
    ...
    private async Task<Guid> SeedBoundRepositoryAsync(Guid teamId, string cloneUrl, string defaultBranch)
    {
        using var scope = _fixture.BeginScope();
        var db = scope.Resolve<CodeSpaceDbContext>();

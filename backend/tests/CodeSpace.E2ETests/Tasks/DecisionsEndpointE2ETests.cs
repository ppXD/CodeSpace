...
/// <summary>
/// E2E coverage for the "Needs decision" queue at <c>GET /api/workflows/decisions</c> through the REAL ASP.NET
/// pipeline — JWT auth, the X-Team-Id team-scope behavior, route binding, the controller, the queue service, the
...
public sealed class DecisionsEndpointE2ETests : IClassFixture<TaskLaunchApiFactory>
{
    ...
    [Fact]
    public async Task List_pending_decisions_resolves_and_is_empty_for_a_fresh_team()
    {
        var (userId, teamId) = await SeedTeamMembershipAsync();

        var response = await SendAsync(HttpMethod.Get, "/api/workflows/decisions", userId, teamId);

        response.StatusCode.ShouldBe(HttpStatusCode.OK, customMessage: await DescribeFailureAsync(response));

        var body = await response.Content.ReadFromJsonAsync<JsonElement[]>();
        body.ShouldNotBeNull();
        body!.ShouldBeEmpty("a fresh team has no pending decisions");
    }

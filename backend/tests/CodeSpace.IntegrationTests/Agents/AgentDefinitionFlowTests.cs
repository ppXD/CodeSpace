...
public class AgentDefinitionFlowTests
{
    ...
    private async Task<(Guid TeamId, Guid UserId)> SeedTeamAsync()
    {
        ...
        db.Team.Add(new Team { Id = teamId, Slug = $"agent-team-{teamId:N}", Name = "Agent Test Team", Kind = TeamKind.Workspace, CreatedBy = SystemUsers.SeederId, LastModifiedBy = SystemUsers.SeederId });

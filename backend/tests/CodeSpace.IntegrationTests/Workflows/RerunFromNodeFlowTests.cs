...
public class RerunFromNodeFlowTests
{
    ...
    {
        using var scope = _fixture.BeginScope();
        return await scope.Resolve<IRunFromSnapshotStarter>().StartFromSnapshotAsync(def, teamId, userId, "{}", scopeRepositoryIds: null, projectionKind: null, CancellationToken.None);
    }

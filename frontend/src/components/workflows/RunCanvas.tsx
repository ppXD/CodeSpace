...
function RunCanvasInner({ definition, runNodes, runStatus, manifestByType }: RunCanvasProps) {
  ...
  const nodes = useMemo<Node<WorkflowNodeData>[]>(
    () => {
      ...
      /* eslint-disable react-hooks/refs -- intentional render-time memo cache for React Flow node reference stability (A3 patchNodes); covered by the render-count test. */

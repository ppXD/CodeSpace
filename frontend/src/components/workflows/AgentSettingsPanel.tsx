...
/**
 * Agent "Settings" — the lifecycle + governance home for one agent: its read-only identity, the
 * Enabled/Paused switch, the (forthcoming) guardrails, and the danger zone. Pure / presentational:
 * the parent loads the workflow and owns the enable/delete handlers (each backed by its own
 * dedicated endpoint, so nothing here touches the canvas definition).
 */
export function AgentSettingsPanel({ workflow, onToggleEnabled, onDelete, toggling = false, deleting = false }: {
  ...
  return (
    <div className="agent-set">
      <header className="agent-set-head">
        ...
        <p className="agent-set-hint">Version v{workflow.latestVersion} · rename and edit the steps in the Source editor.</p>
      </header>

      <section className="agent-set-row">
      ...
      <section className="agent-set-section">
        <h2 className="agent-ov-section-title">Guardrails</h2>
      ...
      <section className="agent-set-row agent-set-danger">
        <div className="agent-set-row-text">

...
// Stub the phase wave (it pulls per-agent hooks) so this test isolates the MERGE + render order.
vi.mock("./TimelinePhase", () => ({
  TimelinePhase: ({ wave }: { wave: { label: string } }) => <div data-testid="wave">{wave.label}</div>,
}));
